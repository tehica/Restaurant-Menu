        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // what action: -> Index
                    // what controller: -> User
                    // where is that controller? in admin area: -> new { area = "Admin" }
                    return RedirectToAction("Index", "User", new { area = "Admin" });

                    /*
                        // for this
                        // example every user what we register will be in ManagerUser Role
                        await _userManager.AddToRoleAsync(user, SD.ManagerUser);
                    */

                    /*
                        if we add only this code after we add role to new registered user
                        we are directly signIn in the user and we are redirecting

                        await _signInManager.SignInAsync(user, isPersistent: false);
                        return LocalRedirect(returnUrl);
                    */

                    // after we add role to user we directly
